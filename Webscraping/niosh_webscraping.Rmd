...
This document will contain a program to webscrape and format NIOSH search data, as well as extract and identify the manufacturing information for each approved manufacturer. The results of this program are not complete and require additional cleaning/manual data entry, but this program should solve the problem of needing to download the NIOSH database. 
...
```{r}
niosh_full <- niosh_appr %>% 
  filter(str_detect(model_no, "\\(FDA\\)", negate = TRUE)) %>% #drop these entries because they are accounted for in the surgical N95 respirator categories of observations
  group_by(company, appr_num) %>% 
  mutate(products = n(), 
    tal = seq(n())) %>% 
  filter(tal == 1) %>% 
  group_by(appr_num) %>% 
  mutate(companies = n()) %>% 
  left_join(niosh_parents) #%>% 
  #mutate(parent = companies > 1 & private_label == "0") %>% 
```
We can then save and export this file. 

```{r}
saveRDS(niosh_full, box_here("niosh_clean.RDS"))
```
...
We now have the approval date for each application. However, there are often multiple companies and countries involved in each application. 

Thus, we need to aggregate our data back to be unique at the company and approval number level. We also want to identify parent companies when a company is part of a broader application. 


```{r}
niosh_parents <- niosh_notes %>% 
  rename(company = private_company_prnt) %>% 
  mutate(company = case_when(
    str_detect(company, "Fido Industrial Safety Masks Manufacturer Company, Ltd.") ~ "Fido Mask Company, Ltd.", 
    TRUE ~ company
  ),
  company = case_when(
    str_detect(company, "Guangzhou Harley Commodity Company, Ltd.") ~ "Guangzhou Harley Commodity Company Ltd.", 
    TRUE ~ company
  ), 
  company = case_when(
    company %in% c("Shining Star Electronic Technology Co., Ltd.") ~ "Shining Star Electronic Technology Co, Ltd.", 
    TRUE ~ company
  ), 
  company = case_when(
    str_detect(company, "Fangtian") ~ "Suzhou Fangtian Industries Company, Ltd.", 
    TRUE ~ company), 
  company = case_when(
    str_detect(company, "Suzhou Sanical") ~ "Suzhou Sanical Protective Product Manufacturing Company, Ltd.", 
    TRUE ~ company
  )) %>%  
  ungroup() %>% 
  mutate(parent = 1) %>% 
