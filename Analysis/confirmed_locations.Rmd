...
sup1 <- thomas_struc %>% 
  ...
  labs(title = "Number of Suppliers by Product and Thomasnet Supplier Type", subtitle = "Thomasnet 05/30/20 - 08/17/20", y = "Unique Thomasnet Suppliers", x = "", fill = "Supplier Type") + 
  theme_bw()
...
```{r}
time_conf <- base2 + 
  geom_area(data = neg_dom, aes(date, n, group = specific_product), position = "stack", fill = "Black") +
  geom_area(data = poss_dom, aes(date, n, group = specific_product), position = "stack", fill = "Light Grey") + 
  geom_area(data = aff_dom, aes(date, n, group = specific_product), position = "stack", fill = "Light Blue") + 
  geom_area(alpha = 0.3, position = "stack", show.legend = FALSE, color = "Black") +
  labs(title = "Confirmed Domestic Manufacturers (Blue), Unknown Manufacturing Entities (Grey), and \nConfirmed Non-Domestic or Non-manufacturing Entities (Black)", subtitle = "Thomasnet 05/30/20 - 08/17/20 \nManufacturers Self-Identifying as: \nProducing Standard Products for FDA Approved, Hospital Grade Masks", y = "Unique Manufacturing Entities", x = "") 
```
