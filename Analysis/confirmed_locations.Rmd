...
sup1 <- thomas_struc %>% 
  ...
  labs(title = "Number of Suppliers by Product and Thomasnet Supplier Type", subtitle = "Thomasnet 05/30/20 - 09/08/20", y = "Unique Thomasnet Suppliers", x = "", fill = "Supplier Type") + 
  theme_bw()
...
all_dom <- bind_rows(aff_nl, aff_nw) %>%
  ...
  mutate(specific_product = if_else(company %in% c("Safe Health", "Luosh USA, LLC"), "Respirators and Face Masks", specific_product)) %>% 
...
conf_dom <- all_dom %>% 
  ungroup() %>% 
  filter(dom_useful == 1) %>% 
  select(company, address, facility_size_dbh, desc, specific_product) %>% 
