...
agg <- read_xlsx(box_here("thomasnet_aggregate.xlsx")) %>% 
  ...
  mutate(previous_week = lag(total, order_by = date), 
         change = (total-previous_week)/previous_week, 
         percent_change = 100*round(change, 2))
...
agg %>% 
  ...
  change_lab = case_when(
    date == ymd("2020-08-17") ~ paste(percent_change, " %", sep = ""))) %>% 
