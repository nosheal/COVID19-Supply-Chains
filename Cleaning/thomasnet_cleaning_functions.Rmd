...
remove_dups <- function(data) {
  ...
  thomas_clean <- anti_join(data, double, by = c("company", "desc")) %>% 
  bind_rows(double)
...
```{r}
thomas_endprods <- bind_rows(fm_resp, thomas_files[[1]]) %>% 
  bind_rows(thomas_files[[2]])
```
...
```{r}
thomas_intprods <- bind_rows(int_prod, thomas_files[[1]]) %>% 
  bind_rows(thomas_files[[2]])
```
