...
remove_dups <- function(data) {
  ...
  count() %>% 
  filter(n == 2) %>% 
  mutate(search_prod = "Face Masks and N95 Respirators") %>% 
  select(-n)
...
cat_3 <- function(data) {
  data %>% 
    ...
    cat_4 = case_when(str_detect(desc, fixed("polypropylene", ignore_case = TRUE)) ~ "cat_4")) %>% 
    mutate(cat_4 = case_when(
      cat_1 == "cat_1" | cat_2 == "cat_2" | cat_3 == "cat_3" ~ NA_character_,
      TRUE ~ cat_4
    ))
...
fda_specs_nw <- function(data) {
  data %>%
    ...
    filter(!is.na(cat_1) | !is.na(cat_2) | !is.na(cat_3) | !is.na(cat_4)) %>% 
  mutate(useful = case_when(
